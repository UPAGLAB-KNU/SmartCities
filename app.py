...
FORMULA_NOTE = ("Z = (원자료 − 평균) ÷ 표준편차 × 방향, T = 50 + 10Z, "
                "백분위 = Z 순위 백분율 (지표별·비교집단 내)")
MAP_NOTE = "시군구 경계: 2025년 기준 229개"
...
def load(sheet_id):
    """읽기 실패 시 마지막 정상 데이터 유지"""
    try:
        raw, ts = fetch(sheet_id)
        st.session_state["last_good"] = (raw, ts)
        return raw, ts, None
    except Exception as e:
        if "last_good" in st.session_state:
            raw, ts = st.session_state["last_good"]
            return raw, ts, e
        return None, None, e
...
if not sheet_id:
    st.error("시트 ID가 설정되지 않았습니다. Streamlit Secrets에 SHEET_ID를 등록하세요.")
    st.stop()

raw, ts, err = load(sheet_id)
if raw is None:
    st.error(f"시트 읽기 실패 — 표시할 데이터가 없습니다. 사유: {err}")
    st.stop()
...
if err is not None:
    st.warning(f"시트 읽기 실패 — 마지막 정상 데이터({ts:%H:%M:%S})를 표시 중입니다. 사유: {err}")
...
if sido == "전체":
    sgg = c5.selectbox("시군구", ["전체"], disabled=True)
else:
    sgg = c5.selectbox("시군구", ["전체"] + sorted(raw[raw["시도명"] == sido]["시군구명"].unique()))
group = c6.selectbox("비교집단",
                     ["전국", "동일 시도", "특별·광역시", "시 지역", "군 지역", "인구규모 유사지역"])
...
if group == "전국":
    ...
    subset = raw[raw["시도명"] == sido] if sido != "전체" else raw
elif group == "인구규모 유사지역":
    if target:
        myg = raw.loc[raw["지역"] == target, "인구규모군"].iloc[0]
        subset = raw[raw["인구규모군"] == myg]
    else:
        subset = raw
else:
    subset = raw[raw["유형구분"] == group]
...
st.caption(f"비교집단: {group} · 유효 지역 {len(sub)}곳 / 전체 {base['지역'].nunique()}곳")
...
if target and not mine.empty:
    val, pct = mine[mode].iloc[0], mine["백분위"].iloc[0]
    cols[5].metric(sgg, f"{val:,.1f}" if mode == "T점수" else f"{val:,.4g}",
                   f"상위 {100-pct:.0f}%")
    val = float(val)
elif target:
    cols[5].metric(sgg, "값 없음")
    val = None
else:
    val = None
...
with left:
    ...
    if val is not None:
        fig.add_vline(x=min(val, CUT), line_color="#1F4E9C", line_width=3,
                      annotation_text=sgg, annotation_position="top")
...
with right:
    ...
    geo = load_geo()
    zmax = CUT
    zmin = 40 if mode == "T점수" else float(sub[mode].min())
    ...
    if target and not mine.empty:
        fig3.add_trace(go.Choropleth(
            ...
            marker_line_color="#D62728", marker_line_width=2.5,
...
if target and not mine.empty:
    ...
    pos_col = f"{group} 내 위치"
    plot[pos_col] = plot["백분위"].apply(
        lambda p: "중간" if 40 <= p <= 60 else
        (f"상위 {100-p:.0f}%" if p > 60 else f"하위 {p:.0f}%"))
    st.dataframe(plot[["항목", "T점수", pos_col, "지표수"]]
                 .style.format({"T점수": "{:.1f}"}),
...
with st.expander("세부지표 전체 보기", expanded=False):
    ...
    if target:
        tbl = (view[view["지역"] == target]
               [["대분류", "지표명", "원자료", "T점수", "백분위"]]
               .sort_values(["대분류", "지표명"]))
        st.caption(f"{target} · {len(tbl)}개 지표")
        st.dataframe(tbl.style.format(
            {"원자료": "{:.2f}", "T점수": "{:.1f}", "백분위": "{:.0f}"}),
            use_container_width=True, hide_index=True, height=520)
    else:
        tbl = (view.pivot_table(index=["시도명", "시군구명"],
                                columns="지표명", values=mode)
               .round(1).reset_index())
        st.caption(f"전체 {len(tbl)}개 지역 × {len(tbl.columns)-2}개 지표 · 값 기준 {mode}")
    ...
    st.download_button("CSV 내려받기",
                       ...
                       file_name=f"smartcity_{cat}_{mode}.csv",
                       mime="text/csv")

st.divider()
st.caption(f"{FORMULA_NOTE} · 데이터 기준 {ts:%Y-%m-%d %H:%M:%S}")
